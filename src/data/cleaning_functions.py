...
class CharFilter(BaseEstimator, TransformerMixin):
    ...
    def _clean_str(self, s):
        if self.log:
            ...
            return ''.join([c if c in self.valid_chars else '' for c in str(s)])
...
class Lemmatize(BaseEstimator, TransformerMixin):
    ...
    def __init__(self):
        ...
        self._lemmatize_str = np.vectorize(self._lemmatize_str,
                                           otypes = [list])
        return
    ...
    def _lemmatize_str(self, s):
        return ' '.join(self.wnl.lemmatize(ti[0], self._get_wordnet_pos(ti[1]))
                        for ti in s)
