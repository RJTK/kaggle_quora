...
from src.conf import S3_BUCKET, S3_DATA_PATH, CHUNKSIZE, Q
...
def filt(s):
    return ''.join([c if c in valid_chars else '' for c in str(s)])
...
def main(test, i_max):
    ...
    if test == 'True':  # Don't chunk
        for f_name in ['train', 'test']:
            ...
            D = clean_chunk(D)
            D.to_csv(
                PROJECT_DIR + '/data/interim/' + f_name + '_test.csv',
                index_label='id')
    else:  # test != 'True'
        pool = Client()
        ...
        for f_name in ['train', 'test']:
            ...
            D0 = clean_chunk(D0)
            D0.to_csv(
                PROJECT_DIR + '/data/interim/' + 'D_tmp.csv',
                mode='w',
                index_label='id')
            del D0
